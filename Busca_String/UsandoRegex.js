...
function TratarTexto(texto, busca){
    regexBusca = new RegExp(busca,'i');
    if (texto.match(regexBusca)) {
        console.log("Texto encontrado");
        ocorrencias = (texto.split(regexBusca).length - 1); //Conta quantas vezes a palavra aparece na string
        posicao = texto.indexOf(busca); //Pega a posicao da primeira ocorrencia da palavra na string
        trecho = texto.substring(posicao-50, posicao+100); //Retorna o trecho da primeira ocorrencia da palavra
